...
#         --p_artifact_to_artifact $p_value &> _nohup/01_m40_t${i}.out &"
#     echo "Running: $cmd"
#     eval $cmd
...
# for i in 70 80 90; do
    python -m model_training.start_training --device cuda:${gpu} --config_file config_files/training/icon/vgg16_microscope-${i}.yaml &> _nohup/train_m${i}.out &
    gpu=$(( (gpu + 1) % 8 ))
done
